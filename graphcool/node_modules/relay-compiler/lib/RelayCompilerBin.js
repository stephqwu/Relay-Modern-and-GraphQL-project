...
let run = (() => {
  var _ref2 = (0, _asyncToGenerator3.default)(function* (options) {
    ...
    var reporter = new (require('graphql-compiler').ConsoleReporter)({
      verbose: options.verbose,
      quiet: options.quiet
    });

    var useWatchman = options.watchman && (yield require('graphql-compiler').WatchmanClient.isAvailable());
    ...
    var parserConfigs = {
      ...
      graphql: {
        baseDir: srcDir,
        getParser: require('graphql-compiler').DotGraphQLParser.getParser,
        getSchema: function getSchema() {
          return schema;
        },
    ...
    var codegenRunner = new (require('graphql-compiler').CodegenRunner)({
      reporter: reporter,
  ...
  return function (_ref) {
    ...
    return new (require('./RelayFileWriter'))({
      config: {
        baseDir: baseDir,
        compilerTransforms: {
          commonTransforms: require('./RelayIRTransforms').commonTransforms,
...
function getSchema(schemaPath) {
  try {
    ...
    if (require('path').extname(schemaPath) === '.json') {
      source = require('graphql').printSchema(require('graphql').buildClientSchema(JSON.parse(source).data));
    }
    source = '\n  directive @include(if: Boolean) on FRAGMENT_SPREAD | FIELD\n  directive @skip(if: Boolean) on FRAGMENT_SPREAD | FIELD\n\n  ' + source + '\n  ';
    return require('graphql').buildASTSchema(require('graphql').parse(source), { assumeValid: true });
  } catch (error) {
